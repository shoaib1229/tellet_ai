...
try:
    ...
    if response.status_code == 200:
        ...
        js_frameworks = {
            ...
            'stripe': 'stripe',
            'firebase': 'firebase',
            'google analytics': 'ga|gtag|googletagmanager',
            'hotjar': 'hotjar'
        }
        ...
        for name, pattern in js_frameworks.items():
            # Skip if already found in script src
            already_found = False
            for detected in detected_js_frameworks:
                if detected.startswith(name + " "):
                    already_found = True
                    break
            
            if not already_found and re.search(pattern, source_code, re.IGNORECASE):
        ...
        for pattern in ajax_patterns:
            if re.search(pattern, source_code):
                ajax_found = True
                # Create safe string without f-string escaping issues
                pattern_clean = pattern.replace('\\', '').replace('(', '').replace('.', '')
                print("  - Found usage of " + pattern_clean.strip())
